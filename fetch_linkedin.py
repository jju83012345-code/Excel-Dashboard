...
def get_stats(share_urn):
    encoded_org = requests.utils.quote(f"urn:li:organization:{ORG_ID}", safe="")
    ...
    res = requests.get(url, headers=headers_rest)
    print(f"[통계 API] {res.status_code} | URN: {share_urn[:40]} | 응답: {res.text[:200]}")
    if res.status_code != 200:
        return {}
